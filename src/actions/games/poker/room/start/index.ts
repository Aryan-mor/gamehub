...
async function handleRoomStart(context: HandlerContext): Promise<void> {
  ...
  const buttons = [
    { text: ctx.t('poker.game.buttons.check'), callback_data: ctx.keyboard.buildCallbackData('games.poker.room.check', { roomId }) },
    { text: ctx.t('poker.game.buttons.call'), callback_data: ctx.keyboard.buildCallbackData('games.poker.room.call', { roomId }) },
    { text: ctx.t('poker.game.buttons.fold'), callback_data: ctx.keyboard.buildCallbackData('games.poker.room.fold', { roomId }) },
  ];
