...
import { generateFormStepKeyboard, generateConfirmationKeyboard } from '../../_utils/formKeyboardGenerator';
...
import { validateRoomForm, checkUserActiveRoom } from '../../_utils/roomValidation';
...
// In-memory form state storage (in production, use Redis or database)
declare global {
 ...
 */
async function handleCreate(context: HandlerContext, _query: Record<string, string> = {}): Promise<void> {
  ...
  try {
    ...
    const hasActiveRoom = await checkUserActiveRoom(user.id.toString());
    if (hasActiveRoom) {
 ...
 */
async function handleConfirmCreate(context: HandlerContext, _query: Record<string, string> = {}): Promise<void> {
  const { user, ctx } = context;
  ...
  try {
    ...
    // Validate form data
    const validation = validateRoomForm(formState.data);
    ...
    // Create room
    const playerId = validatePlayerId(userId);
    ...
    const keyboard = {
      inline_keyboard: [
        ...
        [
          {
            text: '👥 دعوت دوستان',
            switch_inline_query: `join_room_${room.id}`
          },
          {
            text: '📊 اطلاعات روم',
            callback_data: `gpinf?roomId=${room.id}`
          }
        ],
        [
          {
 ...
 */
async function showFormStep(context: HandlerContext, formState: any): Promise<void> {
  const { ctx } = context;
...
register(POKER_ACTIONS.CREATE_ROOM_FORM, handleFormStep, 'Create Room Form Step');
register(POKER_ACTIONS.CREATE_ROOM_CONFIRM, handleConfirmCreate, 'Create Room Confirm');
